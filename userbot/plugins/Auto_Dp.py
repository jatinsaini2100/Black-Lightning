#Plugin from Legend X kanged with giving credits
...
#Lightning Is Not Responsible For Any Ban caused By This, Since You agree all Policy by deploying this bot
...
@borg.on(Lightning_cmd(pattern="batmandp$"))
async def main(event):
    await event.edit("Activating Batman Dp\nEnjoy 💜\n in case of any Problem ask at [Lightning Support](https://t.me/lightning_support_group") 
    while True:
        await animeppbat()
...
@borg.on(Lightnng_cmd(pattern="thordp$"))
async def main(event):
...
async def main(event):
    await event.edit("Activated Actress Dp\nEnjoy 💜 in case of any Problem ask at [Lightning Support](https://t.me/lightning_support_group")
    while True:
...
@borg.on(lightning_cmd(pattern="hackerdp$"))
async def main(event):
...
**Commands found in auto_dp are **
  ...
  •  `.spacedp`
  •  `.wallpapers`
**Function : **__Changes your profile pic every 10 minutes with the command you used \n mean the batman or thor or blah blah blah......)__"""
    }
)
